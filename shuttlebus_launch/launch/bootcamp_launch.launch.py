...
def get_shared_file_path(package_name, folder_name, file_name):
  ...
  return os.path.join(
      get_package_share_directory(package_name), folder_name, file_name)
...
def get_shared_file(package_name, folder_name, file_name, arg_name):
  ...
  file_path = os.path.join(
      get_package_share_directory(package_name), folder_name, file_name)
  return launch.substitutions.LaunchConfiguration(arg_name, default=[file_path])
...
def generate_launch_description():
  ...
  dataspeed_ford_dbw = Node(
      ...
      parameters=[
          get_shared_file('bootcamp_launch', 'config', 'dbw_params.yaml',
                          'params')
      ],
      output='screen')
  ...
  tf = Node(
      ...
      arguments=[
          '0', '0', '0', '0', '0', '0', '1', 'base_link', 'laser_data_frame'
      ])

  main_param_dir = launch.substitutions.LaunchConfiguration(
      'main_param_dir',
      default=os.path.join(
          get_package_share_directory('bootcamp_launch'), 'config',
          'lidarslam.yaml'))
  # Launch Ouster Lidar
  ...
  map_publisher = Node(
      ...
      parameters=[
          launch.substitutions.LaunchConfiguration('map_publisher_param_file')
      ])
  # --------------------------------------------------
  ...
  ndt_localizer = Node(
      ...
      parameters=[
          launch.substitutions.LaunchConfiguration('ndt_localizer_param_file')
      ],
      remappings=[
